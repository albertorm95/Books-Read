...
router.post('/create', function(req, res, next) {
    ...
    Book.createBook(newBook, function(err, book) {
        if(err){
            res.json({success: false, msg:'Failed! Book no created.'});
        }
        else{
            res.json({success: true, msg:'Success! Book created'});
        }
    });
});
...
router.put('/update/:id', function(req, res, next) {
    ...
    Book.updateBook(id, book, function(err){
        if(err){
            ...
            res.json({success: true, msg:'Success! Book updated.'});
        }
    });
});
...
router.delete('/delete/:id', function(req, res, next) {
    ...
    Book.deleteBook(id, function(err) {
        if(err){
            res.json({success: false, msg:'Failed! Book no deleted.'});
        }
        else{
            res.json({success: true, msg:'Success! Book deleted.'});
        }
    });
});
